...
struct chn_conf wind_chn[FS_CHN_NUM] = {
	{
		.index = CH0_INDEX,
		.enable = CHN0_EN,
        .payloadType = PT_H264,
...
int wind_sample_framesource_streamon()
{
	...
	for (i = 0; i < FS_CHN_NUM; i++) {
		if (wind_chn[i].enable) {
			ret = IMP_FrameSource_EnableChn(wind_chn[i].index);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "IMP_FrameSource_EnableChn(%d) error: %d\n", ret, wind_chn[i].index);
				return -1;
			}
		}
	}
...
int wind_sample_framesource_streamoff()
{
	...
	for (i = 0; i < FS_CHN_NUM; i++) {
		if (wind_chn[i].enable){
			ret = IMP_FrameSource_DisableChn(wind_chn[i].index);
			if (ret < 0) {
...
{
	int index = (int)args;
	int chnNum = wind_chn[index].index;
	int i = 0, ret = 0;
	IMPFrameInfo *frame = NULL;
	char framefilename[64];
	int fd = -1;

	if (PIX_FMT_NV12 == wind_chn[index].fs_chn_attr.pixFmt) {
		sprintf(framefilename, "frame%dx%d.nv12", wind_chn[index].fs_chn_attr.picWidth, wind_chn[index].fs_chn_attr.picHeight);
	} else {
		sprintf(framefilename, "frame%dx%d.raw", wind_chn[index].fs_chn_attr.picWidth, wind_chn[index].fs_chn_attr.picHeight);
	}
	...
	ret = IMP_FrameSource_SetFrameDepth(chnNum, wind_chn[index].fs_chn_attr.nrVBs * 2);
	if (ret < 0) {
		IMP_LOG_ERR(TAG, "IMP_FrameSource_SetFrameDepth(%d,%d) failed\n", chnNum, wind_chn[index].fs_chn_attr.nrVBs * 2);
...
int wind_sample_framesource_init()
{
	...
	for (i = 0; i < FS_CHN_NUM; i++) {
		if (wind_chn[i].enable) {
			ret = IMP_FrameSource_CreateChn(wind_chn[i].index, &wind_chn[i].fs_chn_attr);
			if(ret < 0){
				IMP_LOG_ERR(TAG, "IMP_FrameSource_CreateChn(wind_chn%d) error !\n", wind_chn[i].index);
				return -1;
			}

			ret = IMP_FrameSource_SetChnAttr(wind_chn[i].index, &wind_chn[i].fs_chn_attr);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "IMP_FrameSource_SetChnAttr(wind_chn%d) error !\n",  wind_chn[i].index);
				return -1;
			}
		}
	}
...
int wind_sample_framesource_exit()
{
	...
	for (i = 0; i <  FS_CHN_NUM; i++) {
		if (wind_chn[i].enable) {
			/*Destroy channel */
			ret = IMP_FrameSource_DestroyChn(wind_chn[i].index);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "IMP_FrameSource_DestroyChn(%d) error: %d\n", wind_chn[i].index, ret);
				return -1;
			}
		}
	}
...
int wind_sample_jpeg_init()
{
	...
	for (i = 0; i <  FS_CHN_NUM; i++) {
		if (wind_chn[i].enable) {
			imp_chn_attr_tmp = &wind_chn[i].fs_chn_attr;
			...
			/* Create Channel */
			if(direct_switch == 1) {
				if (0 == wind_chn[i].index) channel_attr.bEnableIvdc = true;
			}

			ret = IMP_Encoder_CreateChn(3 + wind_chn[i].index, &channel_attr);
			if (ret < 0) {
			...
			/* Resigter Channel */
			ret = IMP_Encoder_RegisterChn(i, 3 + wind_chn[i].index);
			if (ret < 0) {
...
int wind_sample_encoder_init()
{
	...
	for (i = 0; i <  FS_CHN_NUM; i++) {
		if (wind_chn[i].enable) {
			...
			memset(&channel_attr, 0, sizeof(IMPEncoderCHNAttr));
			enc_attr = &channel_attr.encAttr;
			enc_attr->enType = wind_chn[i].payloadType;
			...
			enc_attr->picWidth = imp_chn_attr_tmp->picWidth;
			enc_attr->picHeight = imp_chn_attr_tmp->picHeight;
            if (wind_chn[i].payloadType == PT_JPEG) {
                chnNum = 3 + wind_chn[i].index;
            } else if (wind_chn[i].payloadType == PT_H264) {
                chnNum = wind_chn[i].index;
			...
			ret = IMP_Encoder_RegisterChn(wind_chn[i].index, chnNum);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "IMP_Encoder_RegisterChn(%d, %d) error: %d\n", wind_chn[i].index, chnNum, ret);
				return -1;
			}
		}
	}
...
int wind_sample_osd_exit(IMPRgnHandle *prHander,int grpNum)
{
	int ret;
...
int wind_sample_get_video_stream()
{
	...
	for (i = 0; i < FS_CHN_NUM; i++) {
		if (wind_chn[i].enable) {
            int arg = 0;
            if (wind_chn[i].payloadType == PT_JPEG) {
                arg = (wind_chn[i].payloadType << 16 | (3 + wind_chn[i].index));
            } else {
                arg = (wind_chn[i].payloadType << 16 | wind_chn[i].index);
            }
			ret = pthread_create(&tid[i], NULL, get_video_stream, (void *)arg);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "Create ChnNum%d get_video_stream failed\n", (wind_chn[i].payloadType == PT_JPEG) ? (3 + wind_chn[i].index) : wind_chn[i].index);
			}
		}
	}
		...
		if (wind_chn[i].enable) {
			...
			for (j = 0; j < NR_JPEG_TO_SAVE; j++) {
				sprintf(snap_path, "%s/snap-%d-%d.jpg",
						SNAP_FILE_PATH_PREFIX, wind_chn[i].index,j);
				...
				ret = IMP_Encoder_PollingStream(3 + wind_chn[i].index, 10000);
				if (ret < 0) {
...
int wind_sample_get_video_stream_byfd()
{
	...
	for (i = 0; i < FS_CHN_NUM; i++) {
        ...
        if (wind_chn[i].enable) {
            ...
            sprintf(stream_path[i], "%s/stream-%d.%s", STREAM_FILE_PATH_PREFIX, chnNum,
                    (wind_chn[i].payloadType == PT_H264) ? "h264" : ((wind_chn[i].payloadType == PT_H265) ? "h265" : "jpeg"));
    ...
    while(1) {
        ...
        FD_ZERO(&readfds);
        for (i = 0; i < FS_CHN_NUM; i++) {
            if (wind_chn[i].enable && saveStreamCnt[i] < totalSaveStreamCnt[i]) {
        ...
        if (ret < 0) {
            ...
            for (i = 0; i < FS_CHN_NUM; i++) {
                if (wind_chn[i].enable && FD_ISSET(vencFd[i], &readfds)) {
                    IMPEncoderStream stream;
