...
#define TAG_NAME "[SIDFE->GET PHOTO]"
...
#endif
static int ParseGetPictureRequest(const uint8_t *msg_buf, uint32_t msg_len, _GetPictureRequest_t *req)
{
    ...
    if (msg_buf[0] != 0xAA || msg_buf[1] != 0x5A || msg_buf[2] != 0x01 || msg_buf[3] != 0xFE)
    {
        return -2;
    }
...
/**
 * 打印 _GetPictureRequest_t 结构体的内容
 * @param request 指向 _GetPictureRequest_t 结构体的指针
 */
static void print_get_picture_request(const _GetPictureRequest_t *request)
{
    if (request == NULL)
    {
    ...
    printf("_GetPictureRequest_t contents:\n");
    printf("Picture ID: %u\n", request->pic_id);
    printf("Offset: %u\n", request->offset);
...
static int PackGetPictureResponseHeader(uint8_t *msg_buf, uint32_t data_payload_len, uint8_t result)
{
    int index = 0;
    msg_buf[index++] = 0xAA;
    msg_buf[index++] = 0x5A;
...
{
    log_i("SIDFE_GetFile............ , Count : %d",send_count);
    send_count++;
    _GetPictureRequest_t req;
    int negative_payload_len = 1;
    ...
    //     return send_negative_resp(resp_buf);
    // }
    char * image_path = "/system/bin/core";
    log_i("find path : %s\n", image_path);
    /* 打开文件 */
    ...
    uint32_t data_payload_len = read_len + 1;
